...
| `database/seeders/BautizoCieloDemoSeeder.php` | Invitación de ejemplo `bautizo-mateo` con la plantilla Bautizo Cielo; idempotente (`php artisan db:seed --class=BautizoCieloDemoSeeder`). | Sumar más fotos reales de bautizo a la galería. |
| `database/seeders/CumpleFiestaDemoSeeder.php` | Invitación de ejemplo `cumple-valeria` con la plantilla Cumpleaños Fiesta; idempotente (`php artisan db:seed --class=CumpleFiestaDemoSeeder`). | Sumar más fotos reales a la galería. |
...
| `resources/css/invitation/themes/xv.css` | Detalles de la plantilla XV Premium (dentro de `.inv-xv`): telón de apertura con corona, marco con esquinas y corona en la portada, destello en el nombre, rombos dorados en separadores y pie con corona. | — |
...
| `resources/css/invitation/themes/bautizo.css` | Tema de la plantilla Bautizo Cielo (todo dentro de `.inv-bautizo`): nubes que se abren al entrar, foto en medallón con halo y paloma, rayos de luz, destellos, burbujas y secciones separadas por olas. | Formas como máscaras SVG que toman la paleta del evento. |
| `resources/css/invitation/themes/cumple.css` | Tema de la plantilla Cumpleaños Fiesta (dentro de `.inv-cumple`): pastel con velas que se soplan al entrar, confeti, globos, banderines entre secciones, la edad en grande y controles con bordes marcados y sombras sólidas. | Los tres colores de fiesta salen del primario, el secundario y su mezcla. |
...
| `resources/lottie-icons/dove-loop-icon.json` | Ícono propio de la paloma del bautizo: aletea, planea y flota entre destellos (generado por `scripts/lottie/build-icons.mjs`). | Encabezado de padrinos en la plantilla de bautizo. |
| `resources/lottie-icons/cake-loop-icon.json` · `balloon-loop-icon.json` | Íconos propios del cumpleaños: pastel con llamas que titilan (cuenta regresiva) y globo que flota (gente especial). Generados por `scripts/lottie/build-icons.mjs`. | — |
...
| `resources/views/invitations/templates/bautizo-cielo.blade.php` | Plantilla pública de bautizo: mismos módulos con diseño, orden y textos propios; los padrinos van primero y el cortejo se presenta como abuelos y tíos. La apertura con nubes se omite en la vista previa, en iframes y tras la primera visita de la sesión. | — |
| `resources/views/invitations/templates/cumple-fiesta.blade.php` | Plantilla pública de cumpleaños: mismos módulos con diseño, orden y textos propios; la edad sale del subtítulo («Mis 30 años»). El pastel de apertura aparece en cada visita salvo en la vista previa y en iframes. | — |
...
| `resources/views/invitations/partials/shell/footer.blade.php` · `intro-script.blade.php` | `footer`: pie común con nombre, fecha, accesos rápidos (confirmar, cómo llegar, agendar, hashtag) y volver al inicio. `intro-script`: decide si se muestra la apertura animada (primera visita de la sesión, fuera de iframes). | — |
...
| `resources/views/invitations/partials/xv/` | Parciales de XV: `crown` (corona SVG que se dibuja), `intro` (telón de apertura) y `glints` (resplandores dorados y destellos de fondo). | — |
| `resources/views/invitations/partials/bautizo/` | Parciales de la plantilla de bautizo: `hero` (medallón y nombre), `dove` (paloma SVG), `cloud` y `clouds-band` (nubes), `intro` (apertura) y `ambient` (nubes y destellos de fondo). | — |
| `resources/views/invitations/partials/cumple/` | Parciales del cumpleaños: `hero` (edad, foto sticker, globos, nombre y entrada), `intro` (pastel y confeti), `balloon`, `bunting` (banderines) y `ambient` (globos y confeti de fondo). | — |
...
| `resources/views/invitations/partials/particles.blade.php` | Capa de partículas con posiciones y tiempos deterministas, para que el HTML no cambie entre visitas y la caché siga funcionando. | — |
