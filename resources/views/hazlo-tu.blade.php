...
@php
    ...
    $benefits = [
        ...
        [
            ...
            'points' => ['Clásicas: '.implode(', ', $collections['clasica']['templates']).'.', 'De temporada: '.implode(', ', $collections['temporada']['templates']).'.', 'Las nuevas de cada evento se suman durante el año, desde el plan Emprendedor.'],
        ],
        [
            'icon' => 'check-circle',
            'title' => 'Confirmación de asistencia',
            'text' => 'Desde Emprendedor, el invitado confirma y te llega su respuesta por WhatsApp. En Agencia, confirma en la invitación y recibe un pase con código QR.',
            'points' => ['Con pase QR ves en tiempo real quién confirmó y quién no respondió.', 'Recoge alergias o restricciones alimentarias.', 'Cada invitación usa una de las dos: la eliges en el editor.'],
        ],
        [
            'icon' => 'qr-code',
            'title' => 'Control de entrada el día del evento',
            'text' => 'En el plan Agencia, quien recibe a los invitados abre un enlace de puerta y escanea el pase de cada uno con la cámara de su teléfono.',
    ...
    $compare = [
        ...
        ['label' => $collections['temporada']['label'], 'value' => fn (array $plan) => in_array('temporada', $plan['collections'] ?? [], true)],
        ['label' => $collections['nueva']['label'], 'value' => fn (array $plan) => in_array('nueva', $plan['collections'] ?? [], true)],
        ['label' => 'Confirmación por WhatsApp', 'value' => fn (array $plan) => in_array('whatsapp', $plan['rsvp'] ?? [], true)],
        ['label' => 'Confirmación con pase QR y control de entrada', 'value' => fn (array $plan) => in_array('pass', $plan['rsvp'] ?? [], true)],
        ['label' => 'Tu marca al pie', 'value' => fn (array $plan) => (bool) ($plan['white_label'] ?? false)],
    ];
    $faqs = [
        ['¿Qué es Hazlo tú?', 'Es un plan mensual que te da tu propio panel en '.$bida['brand'].' para crear invitaciones digitales con nuestras plantillas. Según el plan suma las de temporada, las nuevas de cada evento, la confirmación por WhatsApp o con pase QR y el control de entrada. Pagas el mes y armas las invitaciones que tu plan permite.'],
