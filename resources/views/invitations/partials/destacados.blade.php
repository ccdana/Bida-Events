    ...
    <div class="section-inner-wide space-y-16">
        ...
        <div class="flex justify-center gap-2">
            @foreach(['cortejo' => 'Cortejo', 'padrinos' => 'Padrinos'] as $key => $label)
                <button type="button" @click="section='{{ $key }}'"
                    ...
                    :class="section === '{{ $key }}' ? 'bg-primary text-white border-primary' : 'border-primary/20 opacity-60'">
                    {{ $label }}
                </button>
            @endforeach
        </div>
        ...
        <div x-show="section === 'cortejo'" x-cloak>
            ...
            <div class="mb-14">
                ...
                <div class="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory -mx-6 px-6 scrollbar-hide"
                    x-data="{ active: 0 }">
                    @foreach($destacados['chambelanes'] as $i => $persona)
                        ...
                        <button type="button" @click="active = {{ $i }}"
                            class="snap-center shrink-0 w-36 rounded-2xl border p-5 text-center transition-all duration-300"
                            :class="active === {{ $i }} ? 'border-primary bg-primary/5 scale-105 shadow-md' : 'border-primary/15 bg-white/50 opacity-70'">
                            <div class="w-14 h-14 rounded-full mx-auto bg-primary/15 text-primary font-title text-lg flex items-center justify-center mb-3 transition-transform duration-300"
                                :class="active === {{ $i }} ? 'scale-110 bg-primary text-white' : ''">
                                {{ $iniciales }}
                            </div>
                            <p class="text-sm font-medium leading-tight">{{ $nombre }}</p>
                            @if($detalle)
                                <p class="text-[10px] opacity-50 mt-1" x-show="active === {{ $i }}" x-cloak>{{ $detalle }}</p>
                            @endif
                        </button>
                    @endforeach
                </div>
            ...
            <div>
                <div class="flex items-center justify-center gap-2 mb-6">
                    ...
                    <h3 class="font-title text-xl text-primary">Damitas</h3>
                </div>
                <div class="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory -mx-6 px-6"
                    x-data="{ active: 0 }">
        ...
        <div x-show="section === 'padrinos'" x-cloak x-data="{ open: 0 }">
            <div class="flex items-center justify-center gap-2 mb-8">
                ...
                <h3 class="font-title text-xl text-primary">Nuestros Padrinos</h3>
            </div>
            <div class="space-y-3">
                @foreach($destacados['padrinos'] ?? [] as $i => $padrino)
                    <article class="rounded-2xl border overflow-hidden transition-all duration-300"
                        :class="open === {{ $i }} ? 'border-primary/40 bg-white shadow-md' : 'border-primary/10 bg-white/40'">
                        <button type="button" @click="open = open === {{ $i }} ? -1 : {{ $i }}"
                            ...
                            <span class="shrink-0 w-8 h-8 rounded-full border border-primary/20 flex items-center justify-center transition-transform duration-300"
